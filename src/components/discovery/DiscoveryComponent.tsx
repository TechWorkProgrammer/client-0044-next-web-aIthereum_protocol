...
import {getActiveCategory, saveActiveCategory} from "@/utils/discoveryCategory";
...
const DiscoveryComponent: React.FC = () => {
    ...
    const fetchDataOnce = useCallback(async (url: string, setData: (data: any) => void, setFilteredData: (data: any) => void, category: string) => {
        ...
        try {
            ...
            alert('Internal Server Error', `An error occurred while fetching ${category} data`);
        } finally {
            loader(false);
    ...
    useEffect(() => {
        if (activeCategory === "mesh" && !hasFetched.current.mesh) {
            fetchDataOnce(`${API_BASE_URL}/mesh`, setMeshList, setFilteredMeshList, "mesh").then();
            hasFetched.current.mesh = true;
        } else if (activeCategory === "music" && !hasFetched.current.music) {
            fetchDataOnce(`${API_BASE_URL}/music`, setMusicList, setFilteredMusicList, "music").then();
            ...
            <div className="w-full">
                ...
                {activeCategory === "mesh" ? (
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                        {getPaginatedData().map((mesh) => <MeshCard key={mesh.id} mesh={mesh as MeshData}/>)}
                    </div>
                ) : (
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                        {getPaginatedData().map((music) => <MusicListItem key={music.id} music={music as MusicData}/>)}
                    </div>
                )}
            </div>
            <Pagination
                ...
                totalItems={activeCategory === "mesh" ? filteredMeshList.length : filteredMusicList.length}
