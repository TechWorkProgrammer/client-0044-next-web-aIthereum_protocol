...
const FeatureCard: React.FC<FeatureCardProps> = ({
  ...
  return (
    ...
    >
      ...
      <div className="absolute bottom-0 left-0 w-full bg-primary-900/90 p-2 text-white rounded-b-xl border-t-2 border-accent-400">
        <h3 className="text-sm md:text-xl font-bold mb-2 group-hover:text-accent-400 transition-colors duration-300 px-2">
          {title}
        </h3>
        <p className="text-xs md:text-sm text-secondary-700 mb-4 leading-snug px-2">
          {description}
        </p>
        {isComingSoon ? (
          <Button
